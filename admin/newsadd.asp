...
Set rs=server.createobject("adodb.recordset")
	 sql="select * from lanmu where del=true"
...
<form name="form1" method="post" action="NewsSave.asp?action=add" onSubmit="return checkFields()">
  ...
  <tr bgcolor="#ffffff">
    ...
    <td>&nbsp;<span>
      <select name="lanmuid" id="lanmuid" class="manager_select_1" onChange="list_zj()">
        <% Do While Not RS.EOF %>
        <option value="<%=rs("lanmuid")%>"><%=rs("lanmu")%></option>
        <% RS.MoveNext
		  Loop
		  %>
      </select>
  ...
  <tr bgcolor="#F5F5F5">
    ...
    <td bgcolor="#F5F5F5">&nbsp;
        <input name="source" type="text" size="40" maxlength="90" class="manager_input_1"></td>
  </tr>
  <tr bgcolor="#F5F5F5">
   ...
   <tr bgcolor="#F5F5F5">
    <td width="150" height="25" align="center" bgcolor="#F5F5F5" class="manager_orange_font_12px">发布方式：</td>
    <td>&nbsp;
